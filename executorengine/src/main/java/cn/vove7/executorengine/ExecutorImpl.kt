...
abstract class ExecutorImpl(
    ...
    val globalAutomator = GlobalActionAutomator(getAccessibilityBridge.getBridge(), Handler(Looper.myLooper()))
    ...
    fun openSomething(data: String): PartialResult {
        ...
        return if (pkg != null) {//打开App 解析跟随操作
            val r = checkAccessibilityService()
            if (r.isSuccess) {
                //等待App打开
                val r = waitForApp(pkg)
                if (!r.isSuccess) {
                    return r
                }
                actionQueue = ParseEngine.matchAppAction(data, accessBridge?.currentScope?.packageName
                        ?: "")
                if (actionQueue.isNotEmpty()) Vog.d(this, "openSomething app内操作")
                else Vog.d(this, "openSomething 无后续操作")
                runnable()
                PartialResult(true)
            } else
                r
    ...
    abstract fun checkAccessibilityService(): PartialResult
    abstract fun exec(actionQueue: PriorityQueue<Action>)
    /**
     * 解析打开动作
     */
    ...
    abstract fun runScript(script: String): PartialResult
    /**
     * 解析标识符
     */
    ...
    override fun notifySync() {
        synchronized(lock) {
            lock.notify()
            accessBridge?.removeAllNotifier(this)
        }
    }
    ...
    override fun waitForApp(pkg: String, activityName: String?): PartialResult {
        ...
        accessBridge?.waitForActivity(this, pkg, activityName)
        return if (waitFor())
            PartialResult(true)
    ...
    override fun waitForAppViewId(pkg: String, id: String): PartialResult {
        ...
        accessBridge?.waitForAppViewId(this, pkg, id)
        return if (waitFor())
            PartialResult(true)
        else PartialResult(false, true)
    }
    ...
    override fun waitForViewId(id: String): PartialResult {
        ...
        accessBridge?.waitForViewId(this, id)
        return if (waitFor())
            PartialResult(true)
        else PartialResult(false, true)
    }
    ...
    override fun waitForViewDesc(desc: String): PartialResult {
        ...
        accessBridge?.waitForViewDesc(this, desc)
        return if (waitFor())
            PartialResult(true)
        else PartialResult(false, true)
    }
    ...
    override fun waitForViewText(text: String): PartialResult {
        ...
        accessBridge?.waitForViewText(this, text)
        return if (waitFor())
            PartialResult(true)
        else PartialResult(false, true)
    }
    ...
    fun operateViewOperation(v: String, by: Int, op: Int, v1: String = "",
        ...
        val node = when (by) {
            BY_ID -> accessBridge?.findFirstNodeById(v)
            BY_TEXT -> accessBridge?.findFirstNodeByTextWhitFuzzy(v)
            BY_DESC -> accessBridge?.findFirstNodeByDesc(v)
            BY_ID_AND_TEXT -> accessBridge?.findFirstNodeByIdAndText(v, v1)//id/text
