...
class ActionScriptExecutor(
...
) : AbsExecutorImpl(context, getAccessibilityBridge, systemBridge, serviceBridge) {
    ...
    override fun runnable() {
        ...
        while (actionQueue.isNotEmpty()) {
            ...
            r = runScript(currentAction!!.actionScript)
            when {
                r.needTerminal -> {//出错
                    currentAction = null
     ...
     * 执行Action/Script
     */
    override fun runScript(script: String): PartialResult {
        var execLog = ""
    ...
    override fun exec(actionQueue: PriorityQueue<Action>) {
    ...
    private fun bridgeIsAvailable(): Boolean {
        if (accessApi == null) {
            accessApi = getAccessibilityBridge.getBridge()
            return if (accessApi != null) {
                globalAutomator.setService(accessApi!!)
                true
            } else false
        }
        return true
...
interface GetAccessibilityBridge {
    fun getBridge(): AccessibilityApi?
}
