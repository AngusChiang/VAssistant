...
public class AdbCrypto {
    ...
    public static AdbCrypto get(Context context) throws NoSuchAlgorithmException, IOException, InvalidKeySpecException {
        Pair<File, File> fs = copyKeyPair(context);
        return AdbCrypto.loadAdbKeyPair(fs.getFirst(), fs.getSecond());
    }

    private static Pair<File, File> copyKeyPair(Context context) throws IOException {
        File priFile = new File(context.getFilesDir(), "pri1.key");
        File pubFile = new File(context.getFilesDir(), "pub1.key");
        if (!priFile.exists() || !pubFile.exists()) {
            ByteStreamsKt.copyTo(context.getAssets().open("adb/pri.key"), new FileOutputStream(priFile), ConstantsKt.DEFAULT_BUFFER_SIZE);
            ByteStreamsKt.copyTo(context.getAssets().open("adb/pub.key"), new FileOutputStream(pubFile), ConstantsKt.DEFAULT_BUFFER_SIZE);
        }
        return new Pair<>(priFile, pubFile);
    }
     ...
     */
    public byte[] getAdbPublicKeyPayload(String name) throws IOException {
        ...
        keyString.append(encodeToString(convertedKey));
        keyString.append(" ").append(name);
        keyString.append('\0');
